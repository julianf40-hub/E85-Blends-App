...
//  EightyFiveBlends
//
//  Public SDK key plumbing for the RevenueCat shadow integration (85Blends 2.3.0 Phase 1 — see
//  RevenueCatShadowService.swift for the observer itself, and that file's own header for the
//  full public-vs-secret key distinction). Mirrors SupabaseConfig.swift's established
//  load()/blank-is-missing pattern, this app's one other externally-configured integration.
//
...
enum RevenueCatConfiguration {
    ...
    /// or crashes when this returns `nil` — the shadow integration simply stays disabled.
    ...
    /// `nil` if the key is missing or blank. Never throws — a missing key must only disable the
    /// (non-authoritative, shadow-only) RevenueCat integration, never affect app launch or any
    /// Pro-gated feature. See `RevenueCatShadowService.configureIfNeeded()`, the only caller.
