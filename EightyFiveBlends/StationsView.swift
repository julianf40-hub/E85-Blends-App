...
struct StationsView: View {
    ...
    private var stationsContent: some View {
        ...
        .sheet(isPresented: $isShowingPumpDiagnostics) {
            PumpDetectionDiagnosticsView(savedStations: stations)
        }
    ...
    // branch report for confirmation there's no existing entitlement requirement tying
    // background arrival detection to Pro). Off by default; foreground detection at the
    // pump already works regardless of this toggle (see CalculatorView's existing
    // auto-prompt path) — this only controls background/relaunch delivery.
            ...
            VStack(alignment: .leading, spacing: 12) {
                Toggle(isOn: automaticPumpDetectionToggleBinding) {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Get notified when you arrive at an E85 station, even when 85Blends isn't open. Your location is used only for station arrival features.")
                            .font(.caption)
    ...
    @ViewBuilder
    private var automaticPumpDetectionStatusRow: some View {
        switch pumpDetectionService.status {
        case .disabled:
            EmptyView()
        case .locationNotDetermined:
            automaticPumpDetectionStatusText(
                "Waiting for a location permission response…",
                tint: AppTheme.Colors.textSecondary
            )
        case .whenInUseOnly:
            automaticPumpDetectionStatusBanner(
    ...
    private func automaticPumpDetectionStatusText(_ text: String, tint: Color) -> some View {
        Text(text)
    ...
    private func automaticPumpDetectionStatusBanner(_ text: String, tint: Color, showsSettingsButton: Bool) -> some View {
