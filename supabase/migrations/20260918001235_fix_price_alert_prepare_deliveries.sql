...
-- project (zefkbtscieokkdenvnkg), version 20260918001235, name fix_price_alert_prepare_deliveries,
...
--
-- Supersedes 20260917232144's private.prepare_price_alert_deliveries(uuid) with a per-candidate
-- loop instead of a single CTE-based insert — the CTE form's `cross join lateral` evaluated
-- private.evaluate_price_alert for every candidate row up front, which (per this migration's own
-- ordering relative to 20260918001318 immediately after it) apparently needed to become a
-- per-row loop; see that later migration for the next revision of this same function.
--
